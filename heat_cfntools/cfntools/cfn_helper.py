...
class MetricData(object):
    ...
    def __init__(
            self, access_key=None, secret_key=None, watch_server_path=None):
        self.access_key = access_key
        self.secret_key = secret_key
    ...
    def put_metric_data(self, namespace, name, value=None, timestamp=None,
        ...
        :param statistics: Use a statistic set instead of a value, eg::
        ...
        params.update(self.build_put_params(
            name,
            value=value,
            timestamp=timestamp,
            unit=unit,
    ...
    def build_put_params(self, name, value=None, timestamp=None,
        ...
        def transform(name, value, unit, dimensions, statistics, timestamp):
            tokey = lambda i, key: 'MetricData.member.%d.%s' % (i, key)
            i = count.next()
            yield tokey(i, 'MetricName'), name
            ...
            if statistics:
                ...
                yield tokey(i, 'Value'), value
            else:
                raise Exception('Must specify a value or statistics to put.')

            if dimensions:
                for dim_key, dim_value in self.build_dimension_param(
                        dimensions).iteritems():
                    yield tokey(i, dim_key), dim_value

            if timestamp:
                yield tokey(i, 'Timestamp'), timestamp.isoformat()

            if unit:
                yield tokey(i, 'Unit'), unit

        # map the args with tolist to ensure each arg is a list
        # of the same length, then transform the args into
        # a dict which contains all of the keys/values required
        # for the PutMetricData call
